...
Optional Slurm controls:
  ...
  AF3SCORE_EXCLUDE    Default: empty
...
AF3SCORE_EXCLUDE="${AF3SCORE_EXCLUDE:-}"
...
export AF3SCORE_QOS AF3SCORE_TIME AF3SCORE_EXCLUDE
...
submit_job() {
  ...
  if [[ -n "${AF3SCORE_EXCLUDE:-}" ]]; then
    sbatch_args+=(--exclude="${AF3SCORE_EXCLUDE}")
  fi
  if [[ -n "$nodelist" ]]; then
